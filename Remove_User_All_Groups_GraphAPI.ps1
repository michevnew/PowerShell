    ...
    foreach ($Role in $Roles) {
        ...
        if ($PSCmdlet.ShouldProcess("User $User from role ""$($Role.displayName)""")) {
            ...
            try {
                ...
                Process-Error -ErrorMessage $_ -Group $Role.RoleAssigneeName -User $User
...
function Remove-ExchangeMembership {
    ...
    try {
        ...
        if (!$Group.ExchangeObjectId) { $Group | Add-Member -MemberType NoteProperty -Name "ExchangeObjectId" -Value $result.ExchangeObjectId }
...
foreach ($user in $GUIDs.GetEnumerator()) {
        ...
        if ($ProcessExchangeGroups) {
            ...
            if ($GroupRoleAssignments = $memberOfExchangeRoles | ? {$_.RoleAssigneeType -eq "RoleGroup" -or $_.RoleAssigneeType -eq "SecurityGroup"} | select RoleAssigneeName,RoleAssigneeType -Unique) {
                foreach ($GRA in $GroupRoleAssignments) {
                    ...
                    Remove-ExchangeMembership -Group @{"displayName" = $GRA.RoleAssigneeName;"RecipientType" = $GRA.RoleAssigneeType} -User $User.Name -ExceptionsList $EGUIDs
                }
            }
    ...
    if ($IncludeAdministrativeUnits) {
        ...
        if ($memberOfAUs) { Remove-AUMembership -AUs $memberOfAUs -User $User.Name -ExceptionsList $EGUIDs }
